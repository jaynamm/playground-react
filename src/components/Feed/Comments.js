  ...
  const commentDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post('/api/comment/delete', { id: comment.id },
            {
              headers: {
                'Authorization': localStorage.getItem('Authorization')
              }
        ...
        <div className='w-full'>
          <div className='mb-2'>
            <a className='flex items-center gap-3'>
              <img className='w-7 h-7 object-cover rounded-full border border-solid border-slate-200' src="/user.png" alt="프로필 사진" />
              <div>
                ...
                <p className='text-xs text-slate-700'>
                  ...
                  *
                  <span><Moment format="YYYY-MM-DD HH:mm:ss">{comment.createdDate}</Moment></span>
                </p>
              </div>
              ...
              <div className='relative'>
                {optionsVisible && (
                  <div ref={optionsRef} className='absolute right-0 shadow-lg bg-white rounded border border-slate-300 transform opacity-100 scale-100'>
            ...
            <div className='mb-4 ml-11 pr-5'>
              <div className='w-full'>
                ...
                <div className='mt-2 flex gap-2 justify-end'>
                  <button
                    className='py-1.5 px-3 rounded border text-black text-xs'
